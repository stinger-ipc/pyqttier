...
def example_publish():
    ...
    conn = Mqtt5Connection(
        transport=transport, client_id="pyqttier-example-2-publisher"
    )
    ...
    msg = Message(topic="example/topic", payload=b"Example message", qos=1, retain=True)
...
def example_request_response():
    ...
    def on_response(msg: Message):
        ...
        print(
            f"   Correlation: {msg.correlation_data.decode() if msg.correlation_data else 'None'}"
        )
